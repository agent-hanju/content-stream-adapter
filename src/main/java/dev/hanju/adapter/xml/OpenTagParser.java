 ...
 *
 * <p>속성값은 반드시 따옴표(단일/이중)로 감싸야 합니다. XML 스펙에 따라
 * 따옴표 없는 값은 지원하지 않으며, 만나면 값 없는 속성으로 처리하고 무시합니다.</p>
 *
 * <p><b>사용 예시:</b></p>
 * <pre>{@code
      ...
      switch (state) {
        case AFTER_TAG_NAME:
          if (c == '>') {
            ...
            // self-closing '/'는 속성이 아님 — 무시하고 '>'를 기다림
        ...
        case ATTR_NAME:
          if (c == '=') {
            ...
            // self-closing '/': 현재까지의 이름을 값 없는 속성으로 확정하고 '>' 대기
            if (!currentToken.isEmpty()) {
              attributes.put(currentToken.toString(), "");
              currentToken.setLength(0);
            }
        ...
        case AFTER_ATTR_NAME:
          if (c == '=') {
            ...
            // self-closing '/': 대기 중인 이름을 값 없는 속성으로 확정하고 '>' 대기
            if (currentAttrName != null && !currentAttrName.isEmpty()) {
              attributes.put(currentAttrName, "");
            }
            state = ParserState.AFTER_TAG_NAME;
        ...
        case BEFORE_ATTR_VALUE:
          if (c == '"' || c == '\'') {
            ...
            // unquoted 값은 XML 비적합 — 값을 캡처하지 않고 값 없는 속성으로 처리한 뒤 토큰 스킵
            if (currentAttrName != null) {
              attributes.put(currentAttrName, "");
            }
            currentAttrName = null;
            state = ParserState.SKIP_UNQUOTED_VALUE;
          }
          break;
        ...
        case SKIP_UNQUOTED_VALUE:
          // XML 비적합 unquoted 값 — 경계(공백/'>')까지 문자를 버림
          if (c == '>') {
            return complete(token.substring(i + 1));
          } else if (Character.isWhitespace(c)) {
            state = ParserState.AFTER_TAG_NAME;
